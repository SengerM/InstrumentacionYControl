...
new_samples = [None]*2
new_samples[0] = recorded_samples[0][int(RANCIUS_CYCLES*SAMPLING_FREQUENCY/SIGNAL_FREQUENCY):]*CALIBRATION_FACTOR
new_samples[1] = recorded_samples[1][int(RANCIUS_CYCLES*SAMPLING_FREQUENCY/SIGNAL_FREQUENCY):]*CALIBRATION_FACTOR
#new_samples[1] = new_samples[1] - np.amin(new_samples[1])
...
f.subplots_adjust(hspace=0.3) # Fine-tune figure; make subplots close to each other and hide x ticks for all but bottom plot.
figs.append(f)
axes.plot(new_samples[0], new_samples[1], color=mplt.colors[0])
mplt.beauty_grid(axes)
axes.set_ylabel('Salida')
axes.set_xlabel('Entrada')
# Save figures -----------------------------------
