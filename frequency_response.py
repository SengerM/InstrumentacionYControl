...
def frequency_response(frequencies, amplitude=1, measuring_cycles=100, rancius_time=0.5, sampling_frequency=48000):
	...
	for k in range(len(frequencies)):
		...
		amplitudes[k] = [None]*len(recorded_samples) # Left and right.
		recorded_samples_temp = [None]*len(recorded_samples)
		for l in range(len(recorded_samples)): # Left and right channels.
			recorded_samples_temp[l] = recorded_samples[l][np.int(sampling_frequency*rancius_time):] # Discard rancius samples.
			amplitudes[k][l] = np.max(recorded_samples_temp[l])
		#	amplitudes[k][l] = 2*np.sum(recorded_samples_temp[l]**2)/len(recorded_samples_temp[l])
	return np.transpose(amplitudes)
...
for k in range(len(axes)):
	axes[k].plot(SIGNAL_FREQUENCIES, amplitudes[k], color=mplt.colors[k], label='Channel ' + str(k+1), marker='.')
